...
public class UI implements ActionListener {
    ...
    private final simplejavacalculator.Calculator calc;
    ...
    private simplejavacalculator.BufferedImageCustom imageReturn;

    public UI() throws IOException {
        frame = new JFrame("Calculator PH");

        imageReturn = new simplejavacalculator.BufferedImageCustom();
        ...
        calc = new simplejavacalculator.Calculator();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (checkNum != null || source == butCancel) {
            if (source == butAdd) {
                writer(calc.calculateBi(simplejavacalculator.Calculator.BiOperatorModes.add, reader()));
                text.replaceSelection(butAdd.getText());
            }

            if (source == butMinus) {
                writer(calc.calculateBi(simplejavacalculator.Calculator.BiOperatorModes.minus, reader()));
                text.replaceSelection(butMinus.getText());
            }

            if (source == butMultiply) {
                writer(calc.calculateBi(simplejavacalculator.Calculator.BiOperatorModes.multiply, reader()));
                text.replaceSelection(butMultiply.getText());
            }

            if (source == butDivide) {
                writer(calc.calculateBi(simplejavacalculator.Calculator.BiOperatorModes.divide, reader()));
                text.replaceSelection(butDivide.getText());
            }

            if (source == butxpowerofy) {
                writer(calc.calculateBi(simplejavacalculator.Calculator.BiOperatorModes.xpowerofy, reader()));
            }

            if (source == butSquare) {
                writer(calc.calculateMono(simplejavacalculator.Calculator.MonoOperatorModes.square, reader()));
            }

            if (source == butSquareRoot)
                writer(calc.calculateMono(simplejavacalculator.Calculator.MonoOperatorModes.squareRoot, reader()));

            if (source == butOneDividedBy)
                writer(calc.calculateMono(simplejavacalculator.Calculator.MonoOperatorModes.oneDividedBy, reader()));

            if (source == butCos)
                writer(calc.calculateMono(simplejavacalculator.Calculator.MonoOperatorModes.cos, reader()));

            if (source == butSin)
                writer(calc.calculateMono(simplejavacalculator.Calculator.MonoOperatorModes.sin, reader()));

            if (source == butTan)
                writer(calc.calculateMono(simplejavacalculator.Calculator.MonoOperatorModes.tan, reader()));

            if (source == butlog)
                writer(calc.calculateMono(simplejavacalculator.Calculator.MonoOperatorModes.log, reader()));

            if (source == butln)
                writer(calc.calculateMono(simplejavacalculator.Calculator.MonoOperatorModes.ln, reader()));

            if (source == butrate)
                writer(calc.calculateMono(simplejavacalculator.Calculator.MonoOperatorModes.rate, reader()));

            if (source == butabs)
                writer(calc.calculateMono(simplejavacalculator.Calculator.MonoOperatorModes.abs, reader()));
